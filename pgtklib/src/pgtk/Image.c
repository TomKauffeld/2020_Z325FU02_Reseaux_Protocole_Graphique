...
Image_t* image_init(unsigned int width, unsigned int height)
{
	...
	image->width = width;
	image->height = height;
	image->data.colors = (Color_t*)malloc(sizeof(Color_t) * width * height);
	if (!image->data.colors)
	{
...
Image_t* image_load(const char* filepath)
{
	...
	memcpy(image->data.bytes, uc, (size_t)RGB * width * height);
	stbi_image_free(uc);
	return image;
}
...
Image_t* image_resize(const Image_t* source, unsigned int new_width, unsigned int new_height)
{
	...
	res = stbir_resize_uint8(source->data.bytes, source->width, source->height, 0, destination->data.bytes, new_width, new_height, 0, RGB);
	if (!res)
	{
...
int image_save(const Image_t* image, const char* filepath)
{
	...
	if (strcmp(ext, "png") == 0)
		return stbi_write_png(filepath, image->width, image->height, RGB, image->data.bytes, image->width * RGB);
	if (strcmp(ext, "bmp") == 0)
		return stbi_write_bmp(filepath, image->width, image->height, RGB, image->data.bytes);
	if (strcmp(ext, "jpg") == 0)
		return stbi_write_jpg(filepath, image->width, image->height, RGB, image->data.bytes, JPEG_QUALITY);
	return 0;
}
...
Color_t image_get(const Image_t* image, unsigned int x, unsigned int y)
{
	...
	if (x > image->width || y >= image->height)
		return c;
	return image->data.colors[x + y * image->width];
}
...
void image_set(Image_t* image, unsigned int x, unsigned int y, Color_t c)
{
	if (x >= image->width || y >= image->height)
		return;
	image->data.colors[x + y * image->width].r = c.r;
	image->data.colors[x + y * image->width].g = c.g;
	image->data.colors[x + y * image->width].b = c.b;
}
...
void image_set_r(Image_t* image, unsigned int x, unsigned int y, unsigned char r)
{
	if (x >= image->width || y >= image->height)
		return;
	image->data.colors[x + y * image->width].r = r;
}
...
void image_set_g(Image_t* image, unsigned int x, unsigned int y, unsigned char g)
{
	if (x >= image->width || y >= image->height)
		return;
	image->data.colors[x + y * image->width].g = g;
}
...
void image_set_b(Image_t* image, unsigned int x, unsigned int y, unsigned char b)
{
	if (x >= image->width || y >= image->height)
		return;
	image->data.colors[x + y * image->width].b = b;
}
...
void image_internal_set(Image_t* image, unsigned int x, unsigned int y, unsigned int length, unsigned char c)
{
	if (x + length > image->width || y >= image->height || length < 1)
		return;
	memset(image->data.bytes + x + (unsigned long long)y * image->width, c, (size_t)length * RGB);
}
