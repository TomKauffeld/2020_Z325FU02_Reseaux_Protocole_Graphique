...
Image_t* image_resize(const Image_t* source, unsigned int new_width, unsigned int new_height)
{
	...
	res = stbir_resize_uint8((unsigned char*)source->data, source->width, source->height, 0, (unsigned char*)destination->data, new_width, new_height, 0, RGB);
	if (!res)
	{
...
int image_save(const Image_t* image, const char* filepath)
{
	...
	if (strcmp(ext, "png") == 0)
		return stbi_write_png(filepath, image->width, image->height, RGB, (unsigned char*)image->data, image->width * RGB);
	if (strcmp(ext, "bmp") == 0)
		return stbi_write_bmp(filepath, image->width, image->height, RGB, (unsigned char*)image->data);
	if (strcmp(ext, "jpg") == 0)
		return stbi_write_jpg(filepath, image->width, image->height, RGB, (unsigned char*)image->data, JPEG_QUALITY);
	return 0;
}
